...

{
  ...
  if (!isFleas) {
      if (random < lim1) {
          fleas.add( new Flea(vec2( 1,1)));
          isFleas = true;

      }
  }
  if (isFleas) {
      float distanceTravelled = FLEA_SPEED * elapsedTime;
      fleas[0]->pos = fleas[0]->pos + vec2(0, distanceTravelled);
      random = rand() % 100;
      if (random < lim2) {
          mushrooms.add(new Mushroom(fleas[0]->pos));
      }
      if (fleas[0]->pos.y < WORLD_BOTTOM_ROW) {
          isFleas = false;
  ...
  for (int i=0; i<darts.size(); i++) {
    ...
    vec2 toPoint = seg->pos - darts[i]->pos;
    float distAlongLine = toPoint * seg->dir;
    float distPerpToLine = (toPoint - (distAlongLine * seg->dir)).length();
...

{
  ...
  for (int i=0; i<mushrooms.size(); i++) {
    ...
    vec2 toPoint = mushPos - pos;
    float distAlongLine = toPoint * dir;

    float distPerpToLine = (toPoint - (distAlongLine * dir)).length();
